...
from backend.app.schemas import UserBase, UserInDBBase, TokenDataBase, TokenBase
import os
...
def get_password_hash(password: str):
    return password_hash.has(password)

def get_user(username: str, session: SessionDep) -> UserInDBBase | None:
    try:
        user_model = session.query(UserModel).filter_by(UserModel.username == username).one_or_none()
        if user_model:
            return user_model
...
async def login_for_access_token(
    form_data: Annotated[OAuth2PasswordRequestForm, Depends()], session: SessionDep) -> TokenBase:
...
@router.get("/users/me")
async def rader_users_me(current_user: Annotated[UserBase, Depends(get_current_active_user)]) -> UserBase:
    return current_user

@router.get("/users/me/items/")
async def read_own_items(current_user: Annotated[UserBase, Depends(get_current_active_user)]):
